...
export const CircleImage = ({ sx, src, altText }: ICircleImageProps) => {
  ...
  const sxBase: SxProps<Theme> = useMemo(
    ...
    [theme.palette.divider, theme.palette.primary, theme.palette.secondary, theme.palette.error],
  );

  const combinedSx = { ...sxBase, ...sx };

  return <Box className="mb-2" component="img" src={src} alt={altText} sx={combinedSx} />;
};
